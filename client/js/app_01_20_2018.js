...
function handleTimeClick(timeObj) {
	...
	ga('send', {
	  ...
	  eventCategory: 'Time',
	  eventAction: 'select',
	  dimension1: timeObj.dateString,
	  dimension2: timeObj.time
	});
...
function bookingSuccess(info) {
	...
	ga('send', {
	  ...
	  eventAction: 'successful_booking'
	});
}
...
function eventViewBooking () {
	var el = document.getElementById('booking');
	if (isElementInViewport(el) &&
		GA_FLAGS.booking === undefined) {
		console.log('VIEW_BOOKING');
		ga('send', {
		  hitType: 'event',
		  eventCategory: 'Bookings',
		  eventAction: 'view'
		});
		GA_FLAGS.booking = true;
	}
}

function eventFillForm () {
	if (GA_FLAGS.fillForm === undefined) {
		...
		GA_FLAGS.fillForm = true;
	}
}

/**
 * Setup all of our Google Analytics events
 */
var GA_FLAGS = {};
function setupGAEvents () {
	// View bookings
	eventViewBooking();
	window.addEventListener('scroll', eventViewBooking) ;

	// Begin filling out form
	var form = document.getElementById('booking-info').querySelector('form');
	form.addEventListener('change', eventFillForm);
}
